...
#include "testoscheader.h" //pavan
#include "test.c" //pavan
#include "pthread.h"
...
static void do_exit(VideoState *is)
{
    ...
    CloseHTMSocket(htmsocket);//pavan
    exit(0);
}
...
for (i=0;i<5;i++){
    ...
    x = (j*(src_frame->width/16));
    yc = (i*(src_frame->height/5));
    const unsigned char y = src_frame->data[0][src_frame->linesize[0]*yc + x];

    // U, V components 
    x /= 2;
    yc /= 2;
    const unsigned char u = src_frame->data[1][src_frame->linesize[1]*yc + x];
    const unsigned char v = src_frame->data[2][src_frame->linesize[2]*yc + x];
    ...
    args[index+2].type = INT;
    args[index].datum.i = r;
    args[index+1].datum.i = g;
    args[index+2].datum.i = b;
    
    }
}

   //printf("**%d..%d..%d** \n",r,src_frame->height,src_frame->width);
    
//osc stuff optimize later
//    OSCbuf buf[1];
//    int bundleDepth = 0;
//    OSC_initBuffer(buf, SC_BUFFER_SIZE, bufferForOSCbuf);
//    typedArg arg[240];
...
//    arg.type = INT;
//    arg.datum.i = 1;
    WriteMessage(buf, "/frame", 240, &args);
...
{
    int portnumber = 9109;//pavan
    char *hostname = "192.168.2.74";
...
//    void *htmsocket;
    ...
    //if (argc >= 3 && (strncmp(*argv, "-host", 2) == 0)) {
    //    hostname = argv[1];
    //    argv += 2;
    //    argc -= 2;
    //}
    ...
    if(pthread_create(&inc_x_thread, NULL, main_function, NULL)) {
...
void ParseOSCPacket1(char *buf, int n, ClientAddr returnAddr) {
    ...
    if ((n >= 8) && (strncmp(buf, "#bundle", 8) == 0)) {
        ...
        while(i<n) {
            ...
            ParseOSCPacket1(buf+i+4, size, returnAddr);
            i += 4 + size;
        }
        if (i != n) {
